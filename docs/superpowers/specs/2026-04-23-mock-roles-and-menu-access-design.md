...
- Acceptance checklist：三角色登录 + 菜单可见性 + 403 跳转 + 基于 `/auth/session` 的刷新后权限保持
...
- 角色归一化规则：
  ...
  - `user`、空值 → `user`
  - `viewer` → `viewer`
  - 仅兼容标准英文角色值；非标准值视为无效角色

## 模拟测试账号（仅模拟数据模式生效）

账号/密码说明见 [src/mocks/README.md](../../../src/mocks/README.md)。

行为：

- `/auth/login`：根据邮箱与密码建立 mock 会话，并返回 `user`（含角色）
- `/auth/session`：基于当前 mock 会话返回 `user`（含角色），保证刷新后 `bootstrapAuth()` 能恢复角色状态
...
- 继续保留全局 `beforeEach`：
  ...
  - 已登录访问 `/login`：跳转 redirect 或 `/explore`
  - 命中 `meta.roles` 且 `!canAccess(...)`：跳转 `/403`
- `canAccess` 使用归一化后的角色判定

## 实现拆分

- `src/router/index.ts`：创建 router，并在同文件中导出受保护布局下的子路由数组（包含 `meta.showInMenu` / `meta.roles` / `meta.icon`）
- `src/utils/access.ts`（或同等位置）：提供 `normalizeRole` 与 `canAccess`
- `src/layouts/AdminLayout.vue`：
  - 用 `src/router/index.ts` 中的后台子路由常量生成菜单
  - 统一过滤并渲染
  - 删除零散的权限 `v-if`
- `src/mocks/handlers.ts`：
  - 增加账号表与 mock 会话恢复逻辑
  - `/auth/login`、`/auth/session` 返回角色字段

## 验收标准

- 在模拟数据模式下，使用三组账号登录后：
  - `admin` 可见并可进入：用户管理、系统设置
  - `user/viewer` 不可见：用户管理、系统设置；直接访问 `/admin/users`、`/admin/settings` 会被路由守卫跳回 `/403`
- 菜单项仅通过 routes meta 配置即可完成新增/限制，无需在布局里新增 `v-if`
- 刷新页面后权限状态保持一致（基于 `/auth/session` 恢复角色）
